...
export const cliAuthRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  // === AUTOMATED (NON-INTERACTIVE) AUTHENTICATION ENDPOINTS ===

  // Automated Claude Code authentication with API key
  fastify.post('/api/cli-auth/claude-code/auto', async (request, _reply) => {
    const { apiKey } = (request.body as { apiKey?: string }) || {};
    try {
      logger.info('Starting automated Claude Code authentication');
      const session = await cliAuthService.authenticateClaudeCodeApiKey(apiKey);
      return {
        success: session.status === 'success',
        data: formatSession(session),
        error: session.status === 'failed' ? { code: 'AUTH_FAILED', message: session.message } : undefined,
      };
    } catch (error) {
      logger.error({ error }, 'Failed to auto-authenticate Claude Code');
      return {
        success: false,
        error: { code: 'AUTH_FAILED', message: 'Failed to authenticate Claude Code with API key' },
      };
    }
  });

  // Automated Azure CLI authentication with service principal
  fastify.post('/api/cli-auth/azure-cli/auto', async (_request, _reply) => {
    try {
      logger.info('Starting automated Azure CLI authentication');
      const session = await cliAuthService.authenticateAzureServicePrincipal();
      return {
        success: session.status === 'success',
        data: formatSession(session),
        error: session.status === 'failed' ? { code: 'AUTH_FAILED', message: session.message } : undefined,
      ...
      logger.error({ error }, 'Failed to auto-authenticate Azure CLI');
      return {
        success: false,
        error: { code: 'AUTH_FAILED', message: 'Failed to authenticate Azure CLI with service principal' },
      };
    }
  });

  // Automated gcloud authentication with service account
  fastify.post('/api/cli-auth/gcloud/auto', async (_request, _reply) => {
    try {
      logger.info('Starting automated gcloud authentication');
      const session = await cliAuthService.authenticateGCloudServiceAccount();
      return {
        success: session.status === 'success',
        data: formatSession(session),
        error: session.status === 'failed' ? { code: 'AUTH_FAILED', message: session.message } : undefined,
      ...
      logger.error({ error }, 'Failed to auto-authenticate gcloud');
      return {
        success: false,
        error: { code: 'AUTH_FAILED', message: 'Failed to authenticate gcloud with service account' },
      };
    }
  });

  // Automated authentication for ALL CLI tools at once
  fastify.post('/api/cli-auth/auto-all', async (_request, _reply) => {
    try {
      logger.info('Starting automated authentication for all CLI tools');
      const results = await cliAuthService.authenticateAll();

      const summary = {
        claude: results.claude ? { status: results.claude.status, message: results.claude.message } : null,
        azure: results.azure ? { status: results.azure.status, message: results.azure.message } : null,
        gcloud: results.gcloud ? { status: results.gcloud.status, message: results.gcloud.message } : null,
      };

      const allSuccess =
        (!results.claude || results.claude.status === 'success') &&
        (!results.azure || results.azure.status === 'success') &&
        (!results.gcloud || results.gcloud.status === 'success');
      ...
      return {
        ...
        data: {
          summary,
          sessions: {
            claude: results.claude ? formatSession(results.claude) : null,
            azure: results.azure ? formatSession(results.azure) : null,
            gcloud: results.gcloud ? formatSession(results.gcloud) : null,
          },
        },
      };
    } catch (error) {
      logger.error({ error }, 'Failed to auto-authenticate all CLI tools');
      return {
        success: false,
        error: { code: 'AUTH_ALL_FAILED', message: 'Failed to authenticate CLI tools' },
      };
    }
  });
